...
const standardFeatures = [
  ...
  "Academics and exams management",
  "Financial summary and analytics",
  "AI powered Whatsapp Bot as parent portal",
  "AI powered National Exams Mock Testing (using past questions) and preparation advisory/learning path - This would cut across all countries where we operate",
  "AI assistant for staff, (assessment creation from question bank, business growth metrics, debt and expense management recommendations, etc.)",
  "AI question bank auto create from lesson notes, textbooks and curriculum",
  "16+ modules",
];
const advancedFeatures = [
  "Everything in Standard,",
  "Online assignments & E-notes",
...
export default function PricingSection() {
  ...
  const stdPrice =
    billing === "Termly" ? current.standard.termly : current.standard.yearly;
  const advPrice =
    billing === "Termly" ? current.advanced.termly : current.advanced.yearly;
    ...
    <section id="pricing" className="px-4 py-8">
      <div className="mx-auto flex max-w-9xl flex-col items-center gap-10 rounded-2xl bg-white px-8 py-14 shadow-[0px_1px_4px_0px_rgba(12,12,13,0.1)] lg:px-16">
        ...
        <AnimateIn className="flex flex-col items-center gap-3 text-center">
          ...
          <h2 className="text-4xl leading-tight font-medium tracking-tight text-[#111115]">
        ...
        >
          <p className="text-base font-semibold text-[#111115]">
            ...
            {(["1-200", "201-400", "401+"] as StudentCount[]).map((sc) => (
              <button
                ...
                className={`flex-1 rounded-full py-1.5 text-sm font-medium transition-all ${
                  studentCount === sc
                    ? "border border-zinc-200/70 bg-white text-[#111115] shadow"
                    : "text-[#6f6f77] hover:text-[#111115]"
                }`}
              >
                {sc}
              </button>
            ...
            <div className="overflow-hidden rounded-lg border border-zinc-200">
              <div className="flex flex-col gap-5 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-md border border-zinc-200 bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">
                    Current Plan
                  </span>
                </div>
                <div className="flex items-baseline gap-1">
                  <span className="text-base font-medium text-[#111115]">
                    {stdPrice}
                  </span>
                  <span className="text-xs text-[#6f6f77]">per student</span>
                </div>
                <Link
                  href="https://app.axisbydigenty.com/auth/staff?step=signup"
              ...
              <div className="flex flex-col gap-5 p-6">
                {standardFeatures.map((f) => (
                  <div key={f} className="flex items-start gap-2.5">
                    <Check
                      size={16}
                      className="mt-0.5 shrink-0 text-[#437dfc]"
                    />
