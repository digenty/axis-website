  ...
  return (
    <div className="w-full overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl">
      ...
      <div className="flex items-center justify-between border-b border-zinc-100 bg-zinc-50 px-4 py-2">
        ...
        <span className="text-[11px] font-semibold text-[#111115]">
          Dashboard
        </span>
          ...
          {sidebarSections.map((section, si) => (
            <div key={si} className="mb-1">
              {section.sectionLabel && (
                <p className="px-2 pt-2 pb-0.5 text-[8px] font-semibold tracking-wider text-zinc-400 uppercase">
                  {section.sectionLabel}
                </p>
              )}
            ...
            {[
              {
                ...
                value: "₦200,280",
                tag: "+54%",
                tagCls: "text-green-700 bg-green-50",
              },
              {
                label: "Outstanding Fees",
                value: "₦50,000",
                tag: "+100%",
                tagCls: "text-[#437dfc] bg-blue-50",
              },
              {
                label: "Students",
                value: "1,240",
                tag: "+12%",
                tagCls: "text-violet-700 bg-violet-50",
              },
            ].map((s) => (
              <div
                key={s.label}
              ...
              >
                ...
                <p className="mt-0.5 text-[11px] font-bold text-[#111115]">
                  {s.value}
                </p>
                <span
                  className={`mt-1 inline-block rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${s.tagCls}`}
                >
                  {s.tag}
                </span>
              </div>
          ...
          <div className="flex flex-1 flex-col rounded-xl border border-zinc-100 bg-white p-3">
            ...
            <div className="relative flex flex-1 items-end gap-1 overflow-hidden pb-1">
              {[25, 50, 75, 100].map((pct) => (
                <div
                  key={pct}
                  className="pointer-events-none absolute right-0 left-0 border-b border-zinc-100"
                  style={{ bottom: `${pct}%` }}
                />
              ))}
              {chartBars.map((h, i) => (
                ...
                >
                  <div
                    className="w-full rounded-t-sm bg-zinc-200"
                    style={{ height: `${(100 - h) * 0.8}px` }}
                  />
                  <div
                    className="w-full rounded-t-sm bg-[#437dfc]"
                    style={{ height: `${h * 3}px` }}
                  />
                </div>
              ))}
          ...
          <div className="flex flex-1 flex-col justify-center gap-8 px-8 py-12 lg:px-12 lg:py-20 lg:pr-6">
            <AnimateIn>
              <div className="flex flex-col gap-5">
                ...
                <p className="max-w-[460px] text-base leading-relaxed text-[#4e4e55]">
                  Manage results, CBT exams, fees, attendance, and parent
                  communication in one simple system built for modern schools.
                </p>
              </div>
            </AnimateIn>
            ...
            <AnimateIn delay={150}>
              <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                <Link
                  href="https://app.axisbydigenty.com/auth/staff?step=login"
            ...
            <AnimateIn delay={280}>
              <div className="grid grid-cols-1 gap-3 overflow-hidden md:grid-cols-3">
                {stats.map((s) => (
                  <div
                    key={s.label}
                    className={`flex flex-col gap-1 rounded-xl px-4 py-4 ${s.bg}`}
                  >
                    <span className="text-md font-semibold text-[#111115] sm:text-2xl">
                      {s.number}
                    </span>
                    <span className="text-xs leading-snug text-[#6f6f77] sm:text-sm">
                      {s.label}
                    </span>
                  </div>
                ))}
